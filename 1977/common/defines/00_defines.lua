NDefines = {
	...
	NGame = {
		...
		MESSAGE_TIMEOUT_DAYS = 120,						-- Useful if running the handsoff game. The popup messages that doesn't require the player respond will automatically hide after some timeout.
	...
	NDiplomacy = {
		DIPLOMACY_REQUEST_EXPIRY_DAYS = 120,
	...
	NCountry = {
		...
		AIR_SUPPLY_CONVERSION_SCALE = 0.015,				-- Conversion scale for planes to air supply
	...
	NResistance = {
		...
		GARRISON_MANPOWER_LOST_BY_ATTACK = 0.0175, 	-- Ratio of manpower lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
	...
	NBuildings = {
		...
		BASE_FACTORY_REPAIR = 1.3,			-- Default repair rate before factories are taken into account
	...
	NMilitary = {
		...
		DIVISIONAL_COMMANDER_TRAIT_XP_REQUIREMENT = 300.0,	--Get a trait if any valid options & xp gained >= this
		...
		WAR_SCORE_LEND_LEASE_GIVEN_FUEL_FACTOR = 0.003,  				-- war score gained for every unit of fuel lend lease sent to allies
		WAR_SCORE_LEND_LEASE_RECEIVED_IC_FACTOR = 0.002,  			-- war score deducted for every IC of lend lease received from allies
		WAR_SCORE_LEND_LEASE_RECEIVED_FUEL_FACTOR = 0.002, 			-- war score deducted for every unit of fuel lend lease received from allies
		...
		COMBAT_OVER_WIDTH_PENALTY = -1.0,					-- over combat width penalty per %.
	...
	NAir = {
		...
		COMBAT_DAMAGE_SCALE_CARRIER = 3,					-- same as above but used inside naval combat for carrier battles
		...
		AIR_WING_XP_TRAINING_MISSION_ACCIDENT_FACTOR = 1.0, 				--Training exercises cause more accidents
		...
		THRUST_WEIGHT_SPEED_FACTOR = 0.5,								-- For plane designs, additive agility speed bonus per point of thrust exceeding weight
	...
	NNavy = {
		...
		WAR_SCORE_GAIN_FOR_SUNK_SHIP_PRODUCTION_COST_FACTOR = 0.005,		-- war score gained for every IC of the sunk ship
		WAR_SCORE_GAIN_FOR_SUNK_CONVOY = 0.05,							-- war score gained for every sunk convoy
		WAR_SCORE_DECAY_FOR_BUILT_CONVOY = 0.025,  						-- war score deducted when convoy-raided enemy produces one new convoy
		...
		COMBAT_ARMOR_PIERCING_DAMAGE_REDUCTION = 0.0,					-- All damage reduction % when target armor is >= then shooter armor piercing.
	...
	NAI = {
		...
		DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_LAND = 25,	-- Army XP needed before attempting to create a variant of a type that uses the tank designer (the tank designer DLC feature must be active).
		DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_NAVY = 25,	-- Same as above but for the ship designer.
		DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_AIR = 25,		-- Same as above but for the plane designer.
		DEFAULT_LEGACY_VARIANT_CREATION_XP_CUTOFF_LAND = 25,	-- Army XP needed before attempting to create a variant of a type that uses the legacy upgrades system. ai_strategy supports land_xp_spend_priority upgrade_xp_cutoff. If none is set, this define is used instead.
		...
		MIN_FRONT_SIZE_TO_CONSIDER_STANDARD_COHESION = 12.0,	-- How long should fronts be before we consider switching to standard cohesion (under this, standard cohesion fronts will switch back to relaxed)
